...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "profile-prepped.png")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">devbytarun@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'devbytarun@github:~$ whoami <tspan fill="{INK}">Tarun Khadiya</tspan></text>')
