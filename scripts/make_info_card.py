...
OUT = os.path.join(HERE, "..", "profile-card.svg")
...
ROWS = [
    ("host",),
    ("kv", "Current", "BCA Student @ RIET Jaipur"),
    ("kv", "Focus", "Full Stack Development"),
    ("kv", "Learning", "Java • MERN • DSA"),
    ("kv", "Seeking", "Software Engineering Intern"),
    ("gap",),
    ("sec", "Core Technologies"),
    ...
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building production-ready projects"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">devbytarun@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">devbytarun</tspan><tspan fill="{MUTED}">@</tspan>'
